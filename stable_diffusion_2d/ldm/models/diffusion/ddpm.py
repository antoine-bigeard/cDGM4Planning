...
from stable_diffusion_2d.ldm.util import (
    log_txt_as_img,
    exists,
    default,
    ismap,
...
from stable_diffusion_2d.ldm.modules.ema import LitEma
from stable_diffusion_2d.ldm.modules.distributions.distributions import (
    normal_kl,
    DiagonalGaussianDistribution,
)
from stable_diffusion_2d.ldm.models.autoencoder import (
    VQModelInterface,
    IdentityFirstStage,
    AutoencoderKL,
)
from stable_diffusion_2d.ldm.modules.diffusionmodules.util import (
    make_beta_schedule,
    extract_into_tensor,
    noise_like,
)
from stable_diffusion_2d.ldm.models.diffusion.ddim import DDIMSampler
...
class DDPM2d(pl.LightningModule):
    # classic DDPM2d with Gaussian diffusion, in image space
    def __init__(
        self,
        unet_config,
        timesteps=1000,
...
class LatentDiffusion2d(DDPM2d):
    """main class"""
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "encode") and callable(
                ...
                c = self.cond_stage_model(c[0], src_padding_mask=c[1])
        else:
    ...
    def on_fit_start(self) -> None:
        ...
        val_batch = collate_fn_for_trans(val_fig_batch)
        self.validation_x = val_batch["surfaces"]
        self.validation_y = val_batch["observations"]
    ...
    def on_train_epoch_start(self) -> None:
        ...
        for s in range(self.validation_x[0].size(0)):
            ...
            for k in range(1, len_seq + 1):
                seq.append(
                    self.sample(
                        cond=(
                            self.validation_y[0][s, :k].unsqueeze(0).to(self.device),
                            self.validation_y[1][s, :k].unsqueeze(0).to(self.device),
                        ),
                        batch_size=self.validation_x[0].size(0),
                        shape=self.validation_x[0][[s]].shape,
                    )
                )
        ...
        #         self.sample(
        #             cond=(
        #                 self.validation_y[0][[s]].to(self.device),
        #                 self.validation_y[1][[s]].to(self.device),
        #             ),
        #             batch_size=self.validation_x[0].size(0),
        #             shape=self.validation_x[0][[s]].shape,
        #         )
    ...
    def to_rgb(self, x):
        ...
        x = nn.functional.conv1d(x, weight=self.colorize)
...
class Layout2ImgDiffusion(LatentDiffusion2d):
