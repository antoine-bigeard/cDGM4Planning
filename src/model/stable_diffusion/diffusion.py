...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_time=160):
        super().__init__()
...
def pos_encoding(t, channels):
    inv_freq = 1.0 / (10000 ** (torch.arange(0, channels, 2).float() / channels)).to(
        t.device
    )
    pos_enc_a = torch.sin(t.view(t.size(0), 1).repeat(1, channels // 2) * inv_freq)
    pos_enc_b = torch.cos(t.view(t.size(0), 1).repeat(1, channels // 2) * inv_freq)
    ...
    def __init__(self):
        ...
        self.encoders = nn.ModuleList(
            [
                ...
                SwitchSequential(ResidualBlock(80, 80), AttentionBlock(8, 40)),
                SwitchSequential(ResidualBlock(80, 80), AttentionBlock(8, 40)),
                # SwitchSequential(
        ...
        self.decoders = nn.ModuleList(
            [
                ...
                SwitchSequential(ResidualBlock(320, 160), AttentionBlock(8, 40)),
                SwitchSequential(ResidualBlock(160, 80), AttentionBlock(8, 40)),
            ]
        )
